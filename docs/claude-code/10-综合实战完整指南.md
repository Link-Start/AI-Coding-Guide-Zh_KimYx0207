...
我做综合实战不喜欢堆功能清单，真实项目里最值钱的是需求、代码、测试和交付之间能连起来。
...
下面用 `PreToolUse` 记录即将发起的 MCP 工具调用。事件数据通过 stdin JSON 传入；该记录表示调用请求已到达 Hook，不代表工具已经成功执行。

先创建项目的 `.claude/hooks/` 目录，把下面的 Python 3 脚本保存为 `.claude/hooks/log-tool-event.py`。它先创建日志目录，再用 JSON 编码器写入 UTF-8 JSONL，每行一条记录，文件路径中的引号、反斜杠和换行都会正确转义：
...
# 文件路径属于具体工具的输入；不是通用环境变量。
tool_input = data.get("tool_input", {})
if data["tool_name"] == "Write" and isinstance(tool_input, dict):
    file_path = tool_input.get("file_path")
    if isinstance(file_path, str):
        entry["file_path"] = file_path

for optional_field in ("duration_ms", "is_interrupt"):
...
把下面的事件配置合并进 `.claude/settings.json` 现有的 `hooks` 对象。`command` 应指向本机的 Python 3 可执行文件；`-X utf8` 让 Python 按 UTF-8 读取 stdin。使用 `args` 时，`${CLAUDE_PROJECT_DIR}` 由 Claude Code 展开为项目根目录，每个路径作为完整参数传入：
        ...
        "hooks": [
          {
            ...
            "args": [
              "-X",
              "utf8",
              "${CLAUDE_PROJECT_DIR}/.claude/hooks/log-tool-event.py",
              "${CLAUDE_PROJECT_DIR}/logs/mcp-calls.jsonl"
            ]
          }
        ]
      }
...
复用 3.3.3 节的 `log-tool-event.py`，把以下事件合并到 `.claude/settings.json` 的 `hooks` 对象中。`PostToolUse` 记录成功执行后的工具事件；`PostToolUseFailure` 记录已开始执行但失败的工具事件。这里监控 Bash 和 Write，并把记录追加到同一个 JSONL 文件：
        ...
        "hooks": [
          {
            ...
            "args": [
              ...
              "utf8",
              "${CLAUDE_PROJECT_DIR}/.claude/hooks/log-tool-event.py",
              "${CLAUDE_PROJECT_DIR}/logs/claude-audit.jsonl"
            ]
          }
        ]
      }
...
日志从 stdin 读取 `tool_name`、`hook_event_name`、会话/调用 ID，以及 Write 的 `tool_input.file_path`；事件提供时还记录 `duration_ms` 和 `is_interrupt`。Claude Code 不会自动把这些字段导出成 `$CLAUDE_TOOL_NAME`、`$CLAUDE_TOOL_EXIT_CODE` 或 `$CLAUDE_FILE_PATHS`。`PostToolUse` 的 `tool_response` 结构随工具变化，失败事件的 `error` 是说明字符串，因此脚本按事件名保留结果类别，不猜测统一的工具退出码或返回结构。

这份日志不包含命令正文、文件内容或工具返回全文。权限拒绝发生在执行前，不会触发 `PostToolUseFailure`；需要审计此类拒绝时，应另配 `PermissionDenied`。输入契约见 [官方 PostToolUse](https://code.claude.com/docs/en/hooks#posttooluse-input) 和 [PostToolUseFailure](https://code.claude.com/docs/en/hooks#posttoolusefailure-input)。

#### 3.4.2 审计日志格式

上面的 Hook 直接生成逐行 JSONL。下方是团队自行设计的每日汇总格式，示例文件为 `logs/claude-audit/2025-01-15.json`，也是 3.4.3 节分析脚本要求的输入。使用前需要另外完成 JSONL 到该格式的转换和字段映射；`userId`、分支、差异统计及权限拒绝记录需要各自的数据来源，不由上面的 Hook 自动补齐。
...
[DEBUG] 2025-01-15 10:30:46.002 Model: claude-sonnet-5
...
```
Prompt Caching成本节省（以Sonnet 5为例）：
- 正常输入：$3.0/百万tokens
...
# 仅供演示的费率（美元/百万 tokens），不是现行报价。
# 运行前按所用供应商核价并替换；本示例未计缓存、折扣和地区附加费。
PRICING = {
...
def calculate_daily_cost(log_dir: str, date: str = None):
    ...
    for entry in data.get("entries", []):
        ...
        if model not in PRICING:
            raise ValueError(f"请先为模型 {model!r} 配置核实过的费率")
...
Claude Code 的非交互 `-p` / `--print` 模式支持 `--max-budget-usd`，可以给单次运行设置 API 花费上限。例如：

```bash
claude -p --max-budget-usd 5.00 "概括当前项目的目录结构"
```

v2.1.217+ 会把子代理花费计入上限，并在达到上限后阻止继续派生、停止仍在运行的后台子代理。这个参数只适用于 print 模式，不是所有交互会话的月度账单硬上限；组织级消费仍应在 Console 或网关配置。来源：[CLI 参考](https://code.claude.com/docs/en/cli-reference)（核查日：2026-09-14）。
...
> 以下数值是练习用的监控目标示例，不是厂商承诺或实测结果。实际阈值应在同一模型、网络和任务规模下测量后调整。
