...
## 先修一个小 Bug：空任务列表为什么显示 NaN

项目看板里，完成 2 项、共 4 项，应显示 50%。可新建项目还没有任务时，页面却显示 `NaN%`。先用这个小问题练一遍：把错误交给 Claude，检查它改了哪里，再亲眼看到测试通过。

已装好并登录 Claude Code 后，给自己留一小段练习时间。这个样例只需要 Node.js，不需要安装 npm 依赖；先在终端运行 `node --version` 确认环境可用。还没有 Node.js，可以先做下一节的项目只读分析，等运行环境备好再回来。

### 准备两份练习文件

用编辑器新建一个空目录 `course-progress-lab`，把下面两份文件保存在里面。也可以从本仓库的 [练习目录](../../examples/progress-lab/README.md) 复制文件。它们是本课程的虚构样例，原始代码刻意保留了 Bug。
...
第一次出现 `AssertionError` 是预期现象：空列表得到 `NaN`，测试要求的是 `0`。报错中的路径和行号随你的保存位置变化。

仍在这个目录中运行 `claude`，把下面的任务交给它：

```text
这个练习里，空任务列表的进度会变成 NaN。
请先读 progress.cjs 和 progress.test.cjs，运行 node progress.test.cjs 复现问题，
解释原因，再只修改 progress.cjs，把空列表的结果修成 0。

输入都是非负整数，已完成数不超过总数。
保留正常百分比和四舍五入规则，不删除测试、不加依赖、不改其他目录。
修完运行同一条命令，告诉我实际输出、改动位置和原因。
```

看它给出的改动和命令输出。正常情况下，空列表已经返回 `0`，`2/4`、`4/4`、`1/3` 仍分别返回 `50`、`100`、`33`，最后看到 `progress checks passed`。你也可以自己再运行一次，确认文件确实已经改好。

### 结果不对时，具体指出哪一步

| 你看见什么 | 接着怎么做 |
|---|---|
| 提示找不到 `node` | 先处理运行环境；这是命令没能启动，还没有运行到练习代码 |
| 提示找不到 `progress.test.cjs` | 检查终端当前目录和文件扩展名，避免保存成 `.cjs.txt` |
| Claude 说改好了，却没有测试输出 | 让它执行 `node progress.test.cjs`，报告退出结果和实际输出 |
| 空列表好了，正常进度却全变成 0 | 指出 `2/4` 必须仍为 `50`，要求修复正常分支后重跑测试 |
| 它删掉了空列表检查 | 按上面的原始测试补回这一行，继续修源码；这条检查正是问题的复现方式 |

现在合上任务模板，自己补一个例子：“完成 3 项、共 8 项”。先算出应该是 `38`，再添加检查并运行。换回真实项目时，也照这个顺序提供错误、复现方法、正确结果和允许修改的范围。

做到这里，你已经完成一次可检查的小改动。想让 Claude 下次继续遵守项目规则，再做下面的 `CLAUDE.md` 练习；命令表留到需要时查。工作方法参考 [官方常见工作流程](https://code.claude.com/docs/zh-CN/common-workflows) 和 [最佳实践](https://code.claude.com/docs/zh-CN/best-practices)，样例代码由本课程编写。

---

## 本课学习目标

本章从一处小 Bug 开始，再把项目规则写进 `CLAUDE.md`。你可以先完成这两项，遇到具体操作问题时再查后面的命令和快捷键。

> **2026-06-18 操作口径**：本章继续讲交互、单次执行和打印模式三条主线。v2.1.181 后，交互态、`-p` 和 Remote Control 都可以使用 `/config key=value` 这类配置入口；配置是否生效仍以本机会话里的 `/status`、`/config` 和 `claude --version` 为准。Fable 5 / Mythos 5 相关变化只作为可关注项，不作为本教程默认模型推荐。

> **2026-09-14 当前基线（v2.1.270）**：日常操作有几处变化。v2.1.261 起，`keybindingFlavor` 不再生效，词级编辑统一为 Bash 风格：`Ctrl+W` 删到空白，`Alt+F` / `Alt+D` 停在词尾，标点也分隔单词。已有的 `/diff` 在 v2.1.260 增加全屏侧栏；`/output-style [name]` 可列出和切换输出风格，远程与无头会话也能用。`/skill-doctor` 可查看哪些 Skill 没被使用及其上下文占用。在 `/effort` 选择器里按 `s` 只改本次会话；这不是输入 `/effort s`。待办工具自 v2.1.233 起已对部分新模型默认关闭，v2.1.268 又明确了默认提供的模型范围；需要时设 `CLAUDE_CODE_ENABLE_TODO_TOOLS=1`。全屏下 `Ctrl+L` / `Cmd+K` 清除转录视图，往上滚仍能看旧消息。v2.1.270 修复了长会话里只读 Git 命令意外请求权限的问题。来源：[官方 changelog](https://code.claude.com/docs/en/changelog)。

完成本课学习后，你将能够：

1. **理解三种使用模式**：掌握交互模式、单次执行、打印模式的适用场景
2. **熟练使用交互模式**：掌握REPL对话窗口的完整操作技巧
3. **按任务查找 Slash 命令**：知道何时查看状态、继续会话、调整权限和检查改动
4. **掌握快捷键操作**：提升工作效率的键盘技巧
5. **配置个性化环境**：根据项目需求定制Claude Code
6. **独立排查问题**：使用诊断命令解决大多数常见问题
...
```
✅ 先修一个小 Bug：复现空列表错误，修复后重跑测试
✅ 再写第一份 CLAUDE.md：把真实项目的运行方式和约束留下来
```

**完成后你能带走**：一份修好的练习代码、一条能重复运行的检查命令，以及自己的项目规则。安装和登录时间另计。
...
| **xhigh** | ◉ | 慢 | 多 | **推荐默认值（Opus 4.8）**，深度推理任务 |
| max | ⬤ | 最慢 | 最多 | 极限推理，关键决策 |

> 💡 **推荐**：如果你使用 Opus 4.8 模型，官方推荐默认使用 `xhigh` 级别以获得最佳推理质量。
...
> **2026-08-06 权限模式口径**：v2.1.200 起，原 "Default" 权限模式在 CLI、VS Code、JetBrains 中已统一改名 **"Manual"**（行为不变，只是名字）。教程里只要看到"Default 模式"或"默认权限模式"，按"Manual 模式"理解即可。此外 `/doctor` 已升级为完整诊断入口（原 `/checkup` 别名），`/commit-push-pr` 可在配置好远端后自动允许 git push，`/fork` 现在把会话拷到后台新 session、原会话内的 subagent 改叫 `/subtask`，`/code-review` 改为后台 subagent 运行。
...
| `/ultraplan` | ~~在浏览器里草拟并回传 plan~~（v2.1.222 已移除，改用 `/plan` + 工作流 skill） |
| `/release-notes` | 更新日志 |
| `/bug` | 报告Bug |
...
>
> **更新日期**：2026年9月14日
> **版本**：v1.6（对齐 Claude Code v2.1.270 release 差量）
