...
我写企业落地时会把权限、审计和人员分工放前面，因为企业真正怕的是责任链断掉。

> **2026-06-18 企业治理口径**：v2.1.175 新增 `enforceAvailableModels` 托管设置；v2.1.176 新增 `footerLinksRegexes` 并修复 Hook `if` 路径匹配；v2.1.178 支持 `Tool(param:value)` 参数级权限。企业模板里不要只写“允许/禁止某工具”，还要说明允许哪些模型、哪些参数、哪些路径条件，以及日志里是否会记录工具细节。

> **2026-09-14 当前基线（v2.1.270）**：企业侧这轮有几处是**升级即生效**的破坏性变更，模板要同步改。一是托管设置只要解析失败（managed-settings.json、drop-in、MDM plist、HKLM 值），Claude Code 会**直接拒绝启动**并指出是哪一份，不再静默放行；下发前务必先校验 JSON。二是新增 `managedMcpServers` 托管设置；同时 `allowedMcpServers` 从 v2.1.259 起**只管用户自己加的 server**。以前被你的白名单挡掉的 `managed-mcp.json` 条目，升级后会重新加载，要挡就用 `deniedMcpServers`。三是 `.claude/settings.json` 和 `settings.local.json` 里的 `defaultMode: "bypassPermissions"` 已被忽略（和 `"auto"` 一样），只能写在用户级或托管设置里，或用 `--permission-mode` 传。四是托管了 `forceLoginGatewayUrl` 或 `forceLoginMethod: "gateway"` 的机器，会忽略残留的 API key 和 claude.ai 登录态，强制走网关登录（Bedrock / Vertex / Foundry 不受影响）。另外新增 `maxEffortLevel`，可在顶层或按模型限制 effort 上限，对所有供应商生效。

> **受限执行补充（v2.1.248+；核查日：2026-09-14）**：共享评测机可用 `claude --restricted -p "任务"`。它默认移除运行命令、执行代码和 WebFetch 的内置工具，限制文件访问范围，拒绝 `bypassPermissions`，并忽略用户、项目与本地设置文件；托管设置和显式 `--settings` 仍可加载。因此不能指望项目 `.claude/settings.json` 在此模式下继续生效，也不要把 `--restricted` 当成操作系统级隔离。详见[CLI 参考](https://code.claude.com/docs/en/cli-reference)。
    ...
    steps:
      ...
      - name: Claude代码审查
        ...
        run: |
          ...
          # 调用Claude API进行审查
          # 用 jq 构造 JSON：diff 里通常带引号和换行，直接拼字符串会破坏 JSON
          PAYLOAD=$(jq -n \
            --arg diff "$DIFF" \
            '{
...
{
  "modelOverrides": {
    "claude-sonnet-5": "arn:aws:bedrock:us-east-1:123456:model/claude-sonnet",
    "claude-opus-4-8": "your-azure-deployment-name"
  }
}
```

**工作原理**：
- 设置后，Claude Code内部仍使用标准模型名称（如`claude-sonnet-5`）
...
**文档结束** | 最后更新：2026年9月14日 | 版本：V1.2.5
