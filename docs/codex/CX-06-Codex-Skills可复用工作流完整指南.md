...
2. **判断什么时候升级成Skill**：一个流程重复三次以上、团队多人需要时就该固化
...
Skill 的学习重点不是"会创建一个文件夹"，而是知道什么时候该把一次提示整理成可复用能力。
...
我把 Skills 当作可复用判断，不是简单 prompt 收藏；能复用的必须包含步骤、边界和验收方式。
