...
> **2026-09-13 当前基线（v2026.9.4）**：v2026.9.3 起记忆检索和冷会话更新的开销降下来了，热提示缓存也会保留；v2026.9.2 起回复能在 Gateway 重启后恢复（活动中、排队中和被委派的回复都算），续跑指令在压缩和重试之后也会保留。

> **版本口径（核查日：2026-09-13）**：v2026.7.1-1 修复了 Memory Core 启动冲突导致的 **fatal restart loop**（派生的 legacy-index 与 cache-sidecar 冲突现在可以恢复，不再把 Gateway 困在重启循环里；向量库的结构性损坏仍按可重试处理）。更强的状态安全是后面才到的：v2026.8.1 把已证实损坏的 **quarantine 决定单独存一份**（被隔离的库自身损坏也不会擦掉隔离记录），并提供 `openclaw backup sqlite` 快照的创建/列出/校验/恢复，同时在遇到比当前版本更新的数据库 schema 时**拒绝**竞争性改写而不是反复重启。如果你之前因为记忆相关报错而停用，升级后建议先用 `openclaw doctor` 验证一次再决定是否重新启用。具体行为以本机版本为准。

## 为什么 AI 需要记忆？

我在 OpenClaw 记忆这篇里保留很多边界提醒，是因为我知道记忆一旦写错，后面所有自动化都会被带偏。
...
上下文窗口是 AI 的"工作记忆"，想用好记忆系统，先得理解它怎么工作。
...
v2026.2.22 为 FTS 检索模式新增了西班牙语和葡萄牙语的停用词过滤，改善这两种语言的对话召回：

- 西班牙语
- 葡萄牙语

这两种语言的对话在 FTS 模式下召回更准；记忆文件本身用什么语言写入不受此限制。
