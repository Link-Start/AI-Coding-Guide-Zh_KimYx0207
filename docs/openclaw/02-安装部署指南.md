...
我写安装部署时最看重可复现：你能照着装好，也要知道以后升级时从哪里回头查。

> **2026-09-14 当前基线（v2026.9.4）**：升级链路这几版改动最多，有两条要在升级前先看。一是从 **2026.8.2** 升上来、机器上又没有 service manager 的，先手动跑一次 `openclaw update --no-restart`（v2026.9.1 起才支持无 service 环境继续升级，旧版会直接拒绝）。二是 v2026.9.4 **只有在数据库结构未变、旧包仍兼容且配置校验通过时，才会自动恢复旧包、配置和服务**；数据库迁移、无法确认兼容性或激活后的人工配置修改都可能阻止回滚。迁移后要退回旧版，仍然需要已验证的升级前备份和匹配的 OpenClaw 版本，详见[官方回滚与恢复说明](https://docs.openclaw.ai/install/updating/rollback-and-recovery)。其余改进：v2026.9.1 起 `openclaw update` 补强升级后 Doctor 失败时的 npm candidate 回滚，保留配置和 secret 引用，提供 triage 排障入口，并等插件就绪后再重启；v2026.9.3 起核心和插件改动会先在隔离的 candidate 状态里预演再激活；v2026.9.2 起自动升级会保住当前设置、已启用技能和默认 agent 归属。新装路径也变了：v2026.9.1 起全新安装（含 `npx openclaw@latest`）走快速通道，自动探测本机已有的 Claude Code / Codex 登录态和 API key，在线验证后直接开 web dashboard，完整向导保留为 Custom setup。另外 v2026.8.2 起 Linux 有了桌面端（x86-64 的 `.deb` / AppImage）；v2026.9.4 新增 `OPENCLAW_CONFIG_READONLY=1`，可让 OpenClaw 不再改写由部署方托管的配置。当前 `latest` 为 2026.9.4，`extended-stable` 为 2026.6.35（核查日：2026-09-14）。
...
| 软件 | 最低版本 | 推荐版本 | 用途 |
|------|---------|---------|------|
| Node.js | 24.x 至少 24.16.0，或 26.1.0 及以上 | 26.x（至少 26.1.0） | 运行时环境；25.x 和 26.0.x 不符合当前要求 |
| npm（Node Package Manager，Node.js 的包管理器） | 随所选 Node.js 安装 | 使用随 Node.js 附带的版本 | 包管理器 |
...
# 检查 Node.js 版本
node --version
# 确认版本：v26.1.0 及以上；沿用 24.x 时至少为 v24.16.0

# 检查 npm 版本
npm --version
# 应能输出版本号；无需为了示例单独降级 npm
...
本章按 OpenClaw **v2026.9.4** 安装：新环境推荐 **Node.js 26.x，至少 26.1.0**；已有 24.x 环境可以沿用，但必须达到 **24.16.0**。Node 22、25.x 和 26.0.x 都不符合这个版本的要求。

这个范围来自[官方快速开始](https://docs.openclaw.ai/start/getting-started)和[v2026.9.4 的 `engines.node`](https://github.com/openclaw/openclaw/blob/v2026.9.4/package.json)：`>=24.16.0 <25 || >=26.1.0`（核查日：2026-09-14）。先运行 `node --version`；符合要求就跳过安装，不要反复安装几套 Node。
...
从 [Node.js 官网](https://nodejs.org/) 选择 26.x 安装包（至少 26.1.0）。如果选择 24.x LTS，确认补丁版本不低于 24.16.0；不要只看“LTS”标签就跳过版本检查。
...
核查时 Homebrew 的 `node` 包为 26.x。安装后仍要检查完整版本；如果终端调用到旧版本，用 `which node` 查路径，再选择本节的一种管理方式。
...
> **旧版本说明**：v2026.6.8 时的 Node 22.19+ 兼容说明只适用于当时的版本，不能用于安装本章当前的 v2026.9.4。当前运行时要求见第 2 节；升级和回滚还要对照目标版本、发布通道及备份兼容性。
...
# 安装 Node.js；核查时 Homebrew 的 node 包为 26.x
brew install node
node --version  # 确认符合第 2 节的版本范围
...
# 安装 Node.js 26（NodeSource 方式）
curl -fsSL https://deb.nodesource.com/setup_26.x | sudo -E bash -
sudo apt install -y nodejs

# 验证
node --version  # 确认 v26.x.x，且不低于 v26.1.0
npm --version   # 使用随 Node.js 安装的版本
...
按提示重启电脑，打开 **Ubuntu 终端**，再运行下面的 Bash 命令。Windows 与 WSL 各有自己的 Node 环境，要在 Ubuntu 内检查版本。
...
1. 从 [Node.js 官网](https://nodejs.org/) 下载 26.x Windows 安装包（至少 26.1.0）
...
**症状**：安装提示 `EBADENGINE` / `engine incompatible`，或启动时提示 Node 版本不符合要求。完整报错会随安装方式变化。

v2026.9.4 要求 `>=24.16.0 <25 || >=26.1.0`。比较完整版本号；25.x 虽然大于 24，也不在支持范围内。
...
macOS / Linux 的全局命令一般位于 `npm prefix -g` 输出目录下的 `bin`；原生 Windows 则检查该输出目录本身是否在 Path 中。新版 npm 已移除 `npm bin -g`，不要用它排查。使用 nvm 时，确认当前 shell 已加载 nvm：
...
```text
macOS:
nvm + Node 26（至少 26.1.0）+ npm global + OpenClaw onboard

Linux:
nvm 或 NodeSource + npm global + OpenClaw onboard

Windows:
WSL2 Ubuntu + nvm + Node 26（至少 26.1.0）+ npm global
...
# 2. 仅在版本不满足 >=24.16.0 <25 或 >=26.1.0 时安装
nvm install 26
nvm use 26
...
|---|---|---|---|---|---|---|
| 姓名 | 实际系统 | node --version 的完整输出 | openclaw --version 的输出 | 安装方式 | 实际结果 | 原始报错 |
```
...
第四步，把数据恢复到 volume 或 bind mount。Docker volume / bind mount 的完整写法放在 [09-Docker部署指南](./09-Docker部署指南.md)，安装篇这里只强调迁移原则：新的容器必须挂载同一个持久化数据目录，否则配置、记忆和会话不会自动出现。
