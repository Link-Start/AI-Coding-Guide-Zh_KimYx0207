...
`plugins.allow` 插件级白名单至少自 v2026.2.19 就已存在（当时已会在白名单为空却发现可加载的非捆绑插件时发出启动告警）。v2026.4.5 前后又加了几项相关加固：
...
| 平台 | 动作 |
|---|---|
| Telegram | BotFather 重新生成 token |
| Discord | Developer Portal 重置 bot token |
| Slack | App 管理后台 rotate token |
| WhatsApp/Baileys | 清理本地凭证并重新扫码 |
...
openclaw channels logs <platform-name>
```
...
或者从对应 Agent 的 `skills` 列表里移除它。然后检查：
...
```bash
rg -n "sk-|token|password|secret|AKIA" ~/.openclaw/workspace
```
...
> 建议：订阅 OpenClaw 的 GitHub Security Advisories，第一时间获取安全更新通知。v2026.5.22 已包含 `protobufjs` 8.4.0 安全更新；v2026.6.8 又补充多项运行时、插件、Gateway、Hono 和审计边界。如果你用 Docker 或固定 npm tag，升级后要重新构建镜像并保留 lockfile / shrinkwrap 完整性校验。
